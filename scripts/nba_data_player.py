...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 27 19:19:56 2019
...
def season_player(season,player_id,nba_player_info):
    new_df = pd.DataFrame(columns=nba_player_info['full_name'], index = [season])
    for (i,j) in zip(nba_player_info['full_name'],nba_player_info['id']):
        dict_team = PlayerVsPlayer(player_id=player_id,vs_player_id=j,season=season, headers=headers).get_normalized_dict()
